...
function User(name, age, email, tz, rol, lastconnection) {
  ...
  function UserEnable(usersList, enabledList) {
    ...
    this.addUser = function(useradd, enabledadd) {
      if ((useradd instanceof User) & valuevalid(enabledadd)) {
    ...
    this.setEnabled = function(usertoSearch, valueenabled) {
      if (userInList(usertoSearch) && valuevalid(valueenabled)) {
        if (
          ...
          this.enabledList.includes(usertoSearch) == true
        ) {
        ...
        return console.log("[ERROR] One value is incorrect.");
      }
    };
  ...
  console.log("\x1b[32m", "\n** Insert two users. -----");
  console.log(enableusers.addUser("Cristo", 0)); //<= Prueba de que funciona el método añadir usuario.
  ...
  console.log("\x1b[32m", "\n** Show state of users. -----");
  console.log("- State of user Cristo: " + enableusers.enabledUser("Cristo")); // <= Prueba de que funciona el método mostrar el estado de un usuario.
  ...
  console.log("- Insert the same state: " + enableusers.setEnabled("Dani", 1));
